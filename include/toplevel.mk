...
check-dynamic-def-pkg: FORCE
	@+DEF_PKG_CONFS=""; \
	if [ -f $(TOPDIR)/.config${TASKNAME_SUFFIX} ]; then \
		for config in $(DYNAMIC_DEF_PKG_CONF); do \
			DEF_PKG_CONFS="$$DEF_PKG_CONFS "$$(grep "$$config"=y $(TOPDIR)/.config${TASKNAME_SUFFIX}); \
		done; \
	fi; \
	[ ! -f tmp/${TASKNAME}/.packagedynamicdefault ] || OLD_DEF_PKG_CONFS=$$(cat tmp/${TASKNAME}/.packagedynamicdefault); \
	[ "$$DEF_PKG_CONFS" = "$$OLD_DEF_PKG_CONFS" ] || rm -rf tmp/info/.targetinfo*; \
	mkdir -p tmp/${TASKNAME} && echo "$$DEF_PKG_CONFS" > tmp/${TASKNAME}/.packagedynamicdefault;
